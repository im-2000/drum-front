...
export default class AudioMotionAnalyzer {
  ...
  constructor(container, options = {}) {
    ...
    if (this.useCanvas) this._container.appendChild(canvas);
    ...
    if (this._outNodes.length == 1) {
      for (const i of [0, 1])
        this._analyzer[i].connect(
          !this._stereo && !i ? this._output : this._merger,
          0,
          i
        );
    }
  ...
  getBars() {
    return Array.from(
      this._bars,
      ({ posX, freqLo, freqHi, hold, peak, value }) => ({
        posX,
        freqLo,
        freqHi,
        hold,
  ...
  _calcAux() {
    ...
    this._radius =
      (Math.min(canvas.width, canvas.height) * (this._stereo ? 0.375 : 0.125)) |
      0;
    ...
    this._isAlphaBars =
      this._alphaBars && !this._isLumiBars && this._mode != 10;
    this._isOutline =
      this._outlineBars &&
      this._isOctaveBands &&
      !this._isLumiBars &&
      !this._isLedDisplay;
    this._maximizeLeds =
      !this._stereo || (this._reflexRatio > 0 && !this._isLumiBars);

    this._channelHeight =
      (canvas.height - (isDual && !this._isLedDisplay ? 0.5 : 0)) >> isDual;
    this._analyzerHeight =
      (this._channelHeight *
        (this._isLumiBars || isRadial ? 1 : 1 - this._reflexRatio)) |
      0;
  ...
  _calcBars() {
    ...
    // helper functions
    const binToFreq = (bin) =>
      (bin * this.audioCtx.sampleRate) / this.fftSize || 1; // returns 1 for bin 0
    const barsPush = (posX, binLo, binHi, freqLo, freqHi, ratioLo, ratioHi) =>
      bars.push({
        posX,
        binLo,
        binHi,
    ...
    if (this._isOctaveBands) {
          ...
          if (nBars > 0) {
            ...
            if (diff > 1) {
              ...
              if (
                ...
                prevBar.binLo > bars[nBars - 2].binHi
              ) {
                prevBar.ratioLo = 0;
      ...
      bars.forEach(
        (bar, index) => (bar.posX = initialX + index * this._barWidth)
      );

      minLog = Math.log10(bars[0].freqLo);
      logWidth =
        analyzerWidth / (Math.log10(bars[bars.length - 1].freqHi) - minLog);
    } else {
  ...
  _calcLeds() {
    ...
    if (customParams) {
      ...
      spaceV = Math.min(
        spaceVRatio * dPR,
        Math.max(2, (analyzerHeight / refRatio + 0.1) | 0)
      );
    }
  ...
  _createScales() {
    const freqLabels = [
      ...
      scaleHeight = (Math.min(canvas.width, canvas.height) * 0.03) | 0; // circular scale height (radial mode)
    ...
    canvasR.width = canvasR.height =
      (this._radius << 1) + this._stereo * scaleHeight;

    const radius = canvasR.width >> 1, // this is also used as the center X and Y coordinates of the circular scale canvas
      radialY = radius - scaleHeight * 0.7; // vertical position of text labels in the circular scale
    ...
    for (const freq of freqLabels) {
      ...
      if (x >= 0 && x <= this._analyzerWidth) {
        scaleX.fillText(label, this._initialX + x, canvasX.height * 0.75);
        if (x < this._analyzerWidth)
          // avoid wrapping-around the last label and overlapping the first one
          radialLabel(x, label);

        if (this._mirror) {
          scaleX.fillText(
            label,
            (this._initialX || canvas.width) - x,
            canvasX.height * 0.75
          );
          if (x > 10)
  ...
  _draw(timestamp) {
    ...
    const ledPosY = (height) =>
      ((height * ledCount) | 0) * (ledHeight + ledSpaceV) - ledSpaceV;

    // select background color
    const bgColor =
      !this.showBgColor || (isLedDisplay && !this.overlay)
        ? "#000"
    ...
    let width =
      ...
      (!isOctaveBands
        ? 0
        : Math.max(isLedDisplay ? ledSpaceH : 0, this._barSpacePx));
    ...
    for (let channel = 0; channel < isStereo + 1; channel++) {
      const channelTop = channelHeight * channel + channelGap * channel,
        ...
        analyzerBottom =
          channelTop +
          analyzerHeight -
          (isLedDisplay && !this._maximizeLeds ? ledSpaceV : 0);
        ...
        if (!this.overlay || this.showBgColor) {
          ...
          if (!isRadial || channel == 0)
            ctx.fillRect(
              ...
              channelTop - channelGap,
              analyzerWidth,
              (this.overlay && this.reflexAlpha == 1
                ? analyzerHeight
                : channelHeight) + channelGap
            );
        ...
        if (this.showScaleY && !isLumiBars && !isRadial) {
          ...
          for (let db = maxdB; db > mindB; db -= 5) {
            ...
            if (even) {
              ...
              if (mirrorMode != -1) ctx.fillText(db, scaleWidth * 0.85, labelY);
              if (mirrorMode != 1)
                ctx.fillText(db, canvas.width - scaleWidth * 0.1, labelY);
              ctx.strokeStyle = "#888";
              ctx.setLineDash([2, 4]);
            ...
            ctx.beginPath();
            ctx.moveTo(
              initialX + scaleWidth * even * (mirrorMode != -1),
              ~~posY + 0.5
            ); // for sharp 1px line (https://stackoverflow.com/a/13879402/2370385)
            ctx.lineTo(
              finalX - scaleWidth * even * (mirrorMode != 1),
              ~~posY + 0.5
            );
            ctx.stroke();
          }
      ...
      for (let i = 0; i < nBars; i++) {
        ...
        if (isLedDisplay) {
          ...
          if (barHeight < 0) barHeight = 0; // prevent showing leds below 0 when overlay and reflex are active
        ...
        if (mode == 10) {
          ...
          const nextBarAvg = i
            ? 0
            : ((fftData[this._bars[1].binLo] / 255) *
                maxBarHeight *
                (!isRadial || !channel || -1) +
                barHeight) /
              2;
            ...
            if (i == 0) {
              ...
              if (mirrorMode != -1) {
                const prevFFTData = binLo
                  ? (fftData[binLo - 1] / 255) * maxBarHeight
          ...
          if (isLedDisplay) {
            ...
            ctx.lineTo(
              x,
              isLumiBars ? channelBottom : analyzerBottom - barHeight
            );
            ctx.stroke();
        ...
        if (
          ...
          if (isLedDisplay)
            ...
            // radial - no peaks for mode 10
            radialPoly(
              posX,
              peak * maxBarHeight * (!channel || -1),
              adjWidth,
              -2
            );
        }
      ...
      if (this._reflexRatio > 0 && !isLumiBars) {
        ...
        if (this.reflexFit || isStereo) {
          // always fit reflex in stereo mode
    ...
    if (energy.val >= energy.peak) {
      ...
      else if (energy.peak > 0) energy.peak *= (30 + energy.hold--) / 30; // decay (drops to zero in 30 frames)
    }
  ...
  _makeGrad() {
    ...
    const ctx = this._canvasCtx,
      canvas = ctx.canvas,
      isLumiBars = this._isLumiBars,
      gradientHeight = isLumiBars
        ? canvas.height
        : (canvas.height * (1 - this._reflexRatio * !this._stereo)) | 0,
    ...
    if (this._radial)
      grad = ctx.createRadialGradient(
        ...
        centerX,
        centerY,
        radius - (maxRadius - radius) * this._stereo
      );
    else
      grad = ctx.createLinearGradient(
        ...(isHorizontal
          ? [initialX, 0, initialX + this._analyzerWidth, 0]
    ...
    if (colorStops) {
      ...
      // helper function
      const addColorStop = (offset, colorInfo) =>
        grad.addColorStop(offset, colorInfo.color || colorInfo);
      ...
      for (let channel = 0; channel < 1 + dual; channel++) {
        colorStops.forEach((colorInfo, index) => {
          ...
          let offset =
            colorInfo.pos !== undefined ? colorInfo.pos : index / maxIndex;
          ...
          if (channel == 1) {
            ...
            if (this._radial || isLumiBars) {
              ...
              offset =
                1 -
                (colorInfo.pos !== undefined
                  ? colorInfo.pos
                  : revIndex / maxIndex) /
                  2;
            } else {
              // if the first offset is not 0, create an additional color stop to prevent bleeding from the first channel
              if (index == 0 && offset > 0) addColorStop(0.5, colorInfo);
          ...
          if (this._stereo && index == maxIndex && offset < 0.5)
            addColorStop(0.5, colorInfo);
        });
      }
    }
  ...
  _setCanvas(reason) {
    ...
    canvasX.height = Math.max(
      20 * pixelRatio,
      (Math.min(newWidth, newHeight) / 27) | 0
    );
  ...
  _setProps(options, useDefaults) {
    ...
    const validProps = Object.keys(defaults)
      .filter((e) => e != "start")
      .concat(callbacks, ["height", "width"]);

    // handle deprecated `showLeds` property
    if (
      options &&
      options.showLeds !== undefined &&
      options.ledBars === undefined
    )
